        ...
        if ($this->required || $this->optional) {
            ...
            foreach (array('required','optional') as $type) {
                foreach ($this->$type as $alias => $field) {
                    ...
                    $this->aliases[$alias] = 'http://axschema.edu.tw/' . $field;
    ...
    protected function getAxAttributes()
    {
        ...
        foreach (explode(',', $this->data['openid_signed']) as $key) {
            ...
            if (substr($key, 0, strlen($keyMatch)) == $keyMatch) {
				$key = reset(array_values(explode('.', substr($key, strlen($keyMatch)))));				
				if (isset($this->data['openid_' . $alias . '_type_' . $key])) {				
					if (isset($this->data['openid_' . $alias . '_count_' . $key])) {
						$value = '';
						for ($i=1; $i<=(int) $this->data['openid_' . $alias . '_count_' . $key]; $i++) {
							$value = $value . ',' . $this->data['openid_' . $alias . '_value_' . $key . '_' . $i];
						}
						$value = '[' . substr($value, 1) . ']';
					} else {
						$value = $this->data['openid_' . $alias . '_value_' . $key];	
					}
					$key = substr($this->data['openid_' . $alias . '_type_' . $key], strlen('http://axschema.edu.tw/'));
